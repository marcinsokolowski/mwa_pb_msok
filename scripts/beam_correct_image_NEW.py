...
from ska_pb import config
from ska_pb import mwa_sweet_spots
from ska_pb import primary_beam
from ska_pb import beam_tools
...
def parse_options():
    ...
    parser.add_option('--xyi_file',
                      ...
                      default=None,
                      help="Force xyi_file")
    parser.add_option('--yx_file',
                      dest='yx_file',
                      default=None,
                      help="Force yx_file")
    parser.add_option('--yxi_file',
                      dest='yxi_file',
                      default=None,
                      help="Force yxi_file")
    ...
    parser.add_option('--h5file',
                      dest='h5file',
                      default="EDA2_110_160MHz.h5", # "MWA_embedded_element_pattern_V02.h5",
                      help="H5 file")
    ...
    parser.add_option('--miriad_image',
                      action="store_true",
                      dest="miriad_image",
                      default=True,
                      help="Miriad image with XY and YX image (not XY and XY*)")
...
if __name__ == "__main__":
    ...
    config.set_h5file( options.h5file )
    logger.info("DEBUG : H5 file   = %s / %s" % (options.h5file,config.h5file))
    ...
    logger.info("H5 file   = %s / %s" % (options.h5file,config.h5file))
    ...
    yx_file = options.yx_file
    yxi_file = options.yxi_file
    ...
    yx = None
    if yx_file is not None:
        yx = pyfits.open(yx_file)

    yxi = None
    if yxi_file is not None:
        yxi = pyfits.open(xyi_file)
    ...
    if yx is not None:
        if naxes >= 4:
            data_yx = yx[0].data[0, 0]
        else:
            data_yx = yx[0].data
        yx.close()
    else:
        data_yx = data_xx * 0.00
        print("WARNING : --yx_file not provided -> using ZERO YX image")

    if yxi is not None:
        if naxes >= 4:
            data_yxi = yxi[0].data[0, 0]
        else:
            data_yxi = yxi[0].data
        yxi.close()
    else:
        data_yxi = data_xx * 0.00
        print("WARNING : --yxi_file not provided -> using ZERO YXi image")
    ...
    if options.miriad_image :
        print("WARNING : miriad image -> no imaginary part !!!???")
        Vij = np.array([[data_xx, data_xy + 1j*data_xyi], [data_yx + 1j*data_yxi, data_yy]]) # Question what is MIRIAD XY and YX image ? where should I have j here ?
