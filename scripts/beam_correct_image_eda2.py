...
if __name__ == "__main__":
    ...
    if options.delays is None:
        ...
        options.delays=""
        for ant in range(0,config.N_ANTENNAS) :
           if ant < (config.N_ANTENNAS-1) :
              options.delays += "0,"
           else :
              options.delays += "0"
    ...
    if options.delays is not None and delays is None:
        ...
        try:
            if (',' in options.delays):
                ...
                delays = config.N_ANTENNAS * [int(options.delays)]
        except Exception:
    ...
    if options.miriad_image :
        ...
        Vij = np.array([[data_xx, data_xy], [data_xyi, data_yy]]) # Question what is MIRIAD XY and YX image ? where should I have j here ?
